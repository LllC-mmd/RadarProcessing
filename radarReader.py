...
def MetSTARDataReader(dta_path):
	...
	eleinfo = {}
	radinfo = {}
	f = pd.DataFrame(columns=['radinfo', 'dbt', 'dbz', 'zdr', 'cc', 'phidp', 'kdp'])
...
def getRadarDataByExtent(radar_nc_path, save_path, extent):
	...
	xy_obs_loc = np.concatenate((np.reshape(x_obs_loc.flatten(), (-1, 1)), np.reshape(y_obs_loc.flatten(), (-1, 1))), axis=1)
	...
	xy_center_loc = np.concatenate((np.reshape(x_center_loc.flatten(), (-1, 1)), np.reshape(y_center_loc.flatten(), (-1, 1))), axis=1)
